...
def groups(request):
    if 't_id' in request.session:
        ...
        return render(request, 'group.html',{'grp':grp,'tally':tally})
    return redirect('/')
...
def listjournalvouchers(request,pk):#ann 
    ...
    if m==1:
        ...
        msg1="01-July-22 to 31-July-22"                        
    return render(request,'listjournalvouchers.html',{'journal':j,'msg1':msg1,'cmp':cmp})
...
def stock_items_creation(request):
    if 't_id' in request.session:
        ...
        if request.method=='POST':
            ...
            crt=stock_itemcreation(name=nm,alias=alias,under=under,units=units,godown=godown,batches=batches,trackdate=trackdate,expirydate=expirydate,typ_sply=typ_sply,
            ...
            crt.save()
            return redirect('stock_items_creation')
